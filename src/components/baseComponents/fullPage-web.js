...
(function(root, window, document, factory, undefined) {
  ...
  /* lr: 前一百行都在设置四个模块(section,section nav, slide, slide nav)以及全局使用的内部和外部默认变量 */
  ...
  /**
   * @description 根据参数初始化,并获取container,originals,options的值
   *
  ...
  function initialize(element, customOptions) {
    ...
    var defaults = {
      ...
      loopHorizontal: true,
      //lr: for touch event
      touchSensitivity: 5, //Accessibility
      keyboardScrolling: true,
      recordHistory: true, //design
      controlArrows: true, //Custom selectors
      sectionSelector: SECTION_DEFAULT_SEL,
      slideSelector: SLIDE_DEFAULT_SEL, //events
      //afterLoad & onLeave & 用户回调在什么时刻以怎样的方式实现
      afterLoad: null,
      onLeave: null,
      afterRender: null,
      afterResize: null,
  ...
  /**
   * @description 初始化操作包括:
   * 1. 给container加上WRAPPER类,以供内部使用(什么作用?)
   * 2. 给html加上ENABLED类,以供内部使用(什么作用?)
   * 3. 改变container的样式(有什么作用?)
   * 4. 测试是否支持css3的3d属性,来决定是用transform实现动画还是模拟的jQuery animation
   * 5. 绑定事件
   *  - setMouseWheelScrolling
   *   - >> addMouseWheelHandler
   *   - >> addHandler(绑定wheel事件,并处理了各个浏览器的兼容性问题)
   *   - >> MouseWheelHandler(根据滑动值,决定滑动方向,并以字符串的方式传入scrolling)
   *   - >> scrolling
   *   - >> moveSectionDown/up()
   *   - >> scrollPage(element,callback,isMoveUp)
   *     - 会组装一个v对象,向下传递
   *     - canScroll = false
   *   - >> onleave(activeSection,leavingSection,getNodeIndex(element)+1,getYmovement(element)) & performMovement(v)
   *   - >> 兼容性判断css3的transform功能: transformContainer || scrollTo
   *   - >> afterSectionLoads(v)
   *   - >> afterLoad(v.element, v.anchorLink, v.sectionIndex + 1) && canScroll = true && v.callback
   *  - addTouchHandler
   *  - addResizeHandler
   *  - addScrollHandler
   * 6. 执行afterDomReady函数
  ...
  /**
   * @description Simulates the animated scrollTop of jQuery. Used when css3:false or scrollBar:true or autoScrolling:false
   * http://stackoverflow.com/a/16136789/1081396
   */
  function scrollTo(element, to, duration, callback) {
    // lr: 怎样保证回调在动画执行完后执行的?
    // 由传入的总执行时间(duration),increment,to决定
  ...
  /**
   * @description Getting the position of the element to scroll when using jQuery animations
   */
  ...
  function setUrlHash(url) {
    if (options.recordHistory) {
      ...
      if (isTouchDevice || isTouch) {
        document.history.replaceState(undefined, undefined, '#' + url);
      } else {
  ...
  /*
    ...
    * lr: what's the point of keep track of both states? the states can change the behavior during the function executing?
    */
